...
bool RenderContext::Load(WindowHandle hWindow, uint32_t width, uint32_t height, bool vSyncEnabled, ReloadDesc* pReloadDesc) {
...
void RenderContext::ToggleVSync() {
...
void RenderContext::UpdateDescriptorSet(DescriptorSet* pDescriptorSet, uint32_t index, uint32_t count, DescriptorData* pParams) {
...
RenderContext::Frame RenderContext::BeginFrame() {
    ...
    RenderTarget* pRenderTarget =
        pSwapChain->ppRenderTargets[imageIndex];
    GpuCmdRingElement elem =
        getNextGpuCmdRingElement(&mGraphicsCmdRing, true, 1);
    ...
    return RenderContext::Frame{ mFrameIndex, imageIndex, pRenderTarget, pDepthBuffer, elem };
}
